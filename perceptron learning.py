...
def perceptron_learning(x1, x2, t, alpha, threshold, epochs):
    ...
    for epoch in range(epochs):
        ...
        for i in range(4):
            ...
            print(f'{x1[i]}\t{x2[i]}\t{t[i]}\t{yin:.2f}\t{op}\t{w1:.2f}\t{w2:.2f}\t{b:.2f}')
...
selected_gate = input("Enter gate (AND/OR): ").strip().upper()

if selected_gate in gates:
    # Optional: Auto fill inputs for standard gates
    use_default = input("Use default inputs for logic gates? (y/n): ").strip().lower()
    if use_default == 'y':
        x1 = np.array([0, 0, 1, 1])
        x2 = np.array([0, 1, 0, 1])
    else:
